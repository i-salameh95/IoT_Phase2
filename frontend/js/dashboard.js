...
async function handleStartSimulation() {
    ...
    const simulateEmergency = document.getElementById('simulate-emergency')?.checked || false;
    const emergencyRate = 0.15; // match backend default: 15% of cycles become emergencies
    ...
    setStatus(statusDiv, `Starting monitoring: ${numCycles} cycles...`, 'info');
    ...
    try {
        ...
        for (let i = 0; i < numCycles; i++) {
            ...
            // Run one cycle and immediately update Latest Cycle + alerts
            const cycleEmergency = simulateEmergency && Math.random() < emergencyRate;
            await runCycleAndRefresh(patientId, cycleEmergency);

            // Delay between cycles (unless stopping)
            if (i < numCycles - 1 && isSimulationRunning) {
                await new Promise((r) => setTimeout(r, Math.max(0, delaySeconds * 1000)));
            }
        }

        if (!isSimulationRunning) {
            setStatus(statusDiv, 'Monitoring stopped.', 'info');
        } else {
            setStatus(statusDiv, `Monitoring completed. ${numCycles} cycles.`, 'success');
        }

    } catch (error) {
        setStatus(statusDiv, `Error: ${error.message || 'Monitoring failed'}`, 'error');
    } finally {
        isSimulationRunning = false;
        setUiMode('manual');
...
async function handleStopSimulation() {
    ...
    // Stop the client-side loop immediately
    isSimulationRunning = false;

    // Best-effort: also ask backend to stop, in case something server-side is running
    try {
...
function updateVitalSignCard(measurement, value, value2 = null) {
    const cardMap = {
        ...
        'ambient_temperature': {
            valueId: 'value-room-temp',
            statusId: 'status-room-temp',
            cardId: 'card-ambient-temperature'
        },
...
// -------------------------------
function getMlPredictionForUi(result) {
    if (!result) return null;

    const preds = result.ml_predictions;
    if (!preds || typeof preds !== 'object') return null;

    if (currentPatientId && currentPatientId !== 'all') {
        return preds[currentPatientId] || null;
    }

    const keys = Object.keys(preds);
    return keys.length ? preds[keys[0]] : null;
}

function computeProcessedCountFromReadings(result) {
    const readings = Array.isArray(result?.readings) ? result.readings : [];
    if (!readings.length) return 0;
    return readings.filter(r => r && r.processed_value !== null && r.processed_value !== undefined).length;
}
...
async function updateMlStatus() {
    ...
    try {
        ...
        // Backend uses "best_model"
        algoEl.textContent = status.best_model || '-';
...
// -------------------------------
// Cycle rendering (aligned to backend)
// -------------------------------
function renderLatestCycle(result) {
    const meta = document.getElementById('latest-cycle-meta');
    if (meta) {
        const mlPred = getMlPredictionForUi(result);
        const mlText = mlPred && mlPred.health_status
            ? `${mlPred.health_status} (${Math.round((mlPred.confidence || 0) * 100)}%)`
            : 'N/A';
    ...
    if (decisionsEl) {
        // Backend returns actuator_states
        const decisions = result?.actuator_states || [];
        if (!decisions.length) {
    ...
    if (confidenceEl) {
        const mlPred = getMlPredictionForUi(result);
        if (mlPred && mlPred.health_status) {
            const conf = Math.round((mlPred.confidence || 0) * 100);
            confidenceEl.textContent = `Confidence: ${conf}% (${mlPred.algorithm || 'model'})`;
        } else {
    ...
    if (!Array.isArray(readings) || readings.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5">Run a cycle to see generated data</td></tr>';
        return;
    }

    readings.forEach(r => {
        const rawValue = r.value != null ? r.value : null;
        const processedValue = r.processed_value != null ? r.processed_value : rawValue;

        const statusRaw = rawValue == null ? null : getHealthStatus(r.measurement, rawValue);
        const statusProcessed = processedValue == null ? null : getHealthStatus(r.measurement, processedValue);

        const badges = [];
        if (r.filtered) badges.push('<span class="flag-badge filtered">FILTERED</span>');
        if (r.outlier) badges.push('<span class="flag-badge warning">OUTLIER</span>');
        if (statusRaw) badges.push(`<span class="flag-badge ${statusRaw.class}">RAW ${statusRaw.label}</span>`);
        if (statusProcessed) badges.push(`<span class="flag-badge ${statusProcessed.class}">PROC ${statusProcessed.label}</span>`);

        const rowClass = statusProcessed && statusProcessed.class === 'critical'
            ? 'row-critical'
            : statusProcessed && statusProcessed.class === 'warning'
                ? 'row-warning'
                : '';
      ...
      <td>${r.measurement || ''}</td>
      <td>${r.device_id || ''}</td>
      <td>${rawValue != null ? rawValue : ''}</td>
      <td>${processedValue != null ? processedValue : ''}</td>
      <td>${badges.join(' ')}</td>
    `;
...
function updateFlowMap(result) {
    ...
    const processedCount = computeProcessedCountFromReadings(result);
    const mlPred = getMlPredictionForUi(result);
    ...
    mlEl.textContent = mlPred && mlPred.health_status
        ? `${mlPred.health_status} (${Math.round((mlPred.confidence || 0) * 100)}%)`
        : 'No prediction';

    const activated = result.actuators_activated ?? 0;
    actuatorsEl.textContent = `${activated} active actuator(s)`;
    ...
    const mlState = mlPred && mlPred.health_status === 'Critical'
        ? 'critical'
        : mlPred && mlPred.health_status === 'Warning'
            ? 'warning'
            : 'active';
    setState(mlEl, mlState);

    setState(actuatorsEl, activated > 0 ? 'warning' : 'active');
}
...
function renderModelComparison(result) {
    ...
    rows.forEach((row) => {
        ...
        labels.push(row.algorithm || 'model');
