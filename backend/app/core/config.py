...
class Settings:
    ...
    def __init__(self):
        ...
        self.MONGODB_URL = _get_django_setting('MONGODB_URL') or os.getenv('MONGODB_URL', 'mongodb://admin:admin123@localhost:27017/health_data?authSource=admin')
        self.MONGODB_DATABASE = _get_django_setting('MONGODB_DATABASE') or os.getenv('MONGODB_DATABASE', 'health_data')

        # MQTT Configuration
        self.MQTT_ENABLED = (_get_django_setting('MQTT_ENABLED') or os.getenv('MQTT_ENABLED', 'False')) == 'True'
        ...
        self.MQTT_TOPIC_PROCESSED = _get_django_setting('MQTT_TOPIC_PROCESSED') or os.getenv('MQTT_TOPIC_PROCESSED', 'iot/health/processed/#')
        self.MQTT_QOS = int(_get_django_setting('MQTT_QOS') or os.getenv('MQTT_QOS', '1'))
        self.MQTT_CONNECT_RETRIES = int(_get_django_setting('MQTT_CONNECT_RETRIES') or os.getenv('MQTT_CONNECT_RETRIES', '20'))
        self.MQTT_CONNECT_DELAY = float(_get_django_setting('MQTT_CONNECT_DELAY') or os.getenv('MQTT_CONNECT_DELAY', '1.0'))
